...
## 46. FinOps / Token Economics reference — extends the AI router in place, illustrative bands only

**Decision:** New 7th input on the AI Governance & Feasibility Router, `#ai-model-tier` (Deterministic / N/A, Lightweight / Low-Latency, Mid-Tier, Frontier / Reasoning — deliberately generic tier names, never named commercial models, since real vendor pricing goes stale and this static app has no way to keep it current). New `js/config/tcoModel.js` supplies one reference entry per tier — illustrative `$/1K tokens` band, illustrative latency band, and when-to-use guidance — rendered as a table on the AI tab with the selected tier's row highlighted. Every entry and the file's header comment state explicitly: **"illustrative order-of-magnitude reference bands, not real provider pricing — verify against current published pricing before using in a real business case,"** the same phrasing `dor-recovery-console`'s `costModel.js` already uses for its own illustrative figures. One new hazard rule, `frontier-reasoning-bounded-latency` (Med), fires when Frontier/Reasoning is paired with a Bounded latency/cost budget — the one place this input changes the verdict rather than just informing the reference table. Follows `DECISIONS.md` #29's precedent: extend the existing router in place, not a parallel intake.

**Why:** Requested as part of a director-level pitch to deepen the AI Governance Router with FinOps/Token Economics coverage. A cost/latency reference is useful context for the PROCEED / PROCEED WITH CONDITIONS / RECONSIDER APPROACH decision, but only if it's honest about being a reference band and not a real pricing feed — this app has no live pricing source and no backend to keep one current, so fabricating specific numbers would violate the standing "never fabricate a capability" discipline. Verified via headless browser: all 4 tiers render, selecting Frontier/Reasoning + Bounded correctly triggers and highlights the Latency/Cost Tier Mismatch hazard, and Reset correctly returns the select to its default tier and clears the hazard (exercising the Reset-completeness fix from #43/#44).

**Trade-off:** The reference bands will drift from real-world pricing over time, same as `costModel.js`'s. Mitigated the same way — order-of-magnitude bands plus an explicit disclaimer, not a claim of currency.

---

## 47. Deeper Agentic Risk — blast-radius and recursive execution, as declared-property hazard rules

**Decision:** New 8th input on the AI Governance & Feasibility Router, `#ai-agent-orchestration` (Single Agent, Multi-Agent Orchestration, Recursive Tool Chaining). Two new hazard rules in `aiHazardRules.js`, both High-severity and both requiring mutating (`read-write-mcp`) tool access to co-trigger: `agentic-recursive-chaining-mutating` ("Recursive Tool Execution Hazard" — each hop compounds what one poisoned response can reach; requires a hard step/depth ceiling and a kill-switch) and `agentic-multi-agent-mutating` ("Multi-Agent Blast-Radius Hazard" — requires least-privilege credential scoping per agent and one accountable HITL approval point spanning all agents, not per-agent approval that can be individually bypassed). Read-only tool access with either orchestration shape triggers neither rule — the hazard is specifically about what a *mutating* agent can do once chained or multiplied, not about orchestration shape alone. Same "declaration, not interception" discipline as the two existing MCP rules (`DECISIONS.md` #32): this static app cannot observe a real execution graph, only ask the assessor to declare it.

**Why:** Requested as part of the same director-level pitch as #46, specifically to deepen Agentic Risk coverage beyond the existing single-hop Confused Deputy / Unvalidated Tool Output rules toward blast-radius and recursive-execution hazards. Explicitly **not** real static/dynamic analysis — `DECISIONS.md` #28 already defers that as needing a backend this project deliberately doesn't have, and this addition must not quietly reverse that. Verified via headless browser: read-write access + Recursive Tool Chaining triggers the Recursive Tool Execution Hazard (alongside the pre-existing Confused Deputy rule, which independently matches on tool access alone); read-write + Multi-Agent Orchestration triggers the Multi-Agent Blast-Radius Hazard; read-only access with either orchestration shape triggers neither new rule; Reset correctly returns the select to Single Agent and clears both hazards.

**Trade-off:** Two more self-reported inputs an assessor could answer inaccurately (deliberately or not) — same limitation as every other hazard rule in this table, mitigated the same way: the flag and its guidance are visible and reviewable, not a silent gate.

---

## 48. OWASP LLM Top 10 / NIST AI RMF coverage map — a reference, not a certification

**Decision:** New `js/config/owaspNistMap.js`: a static lookup, `HAZARD_RULE_FRAMEWORK_MAP`, from each `aiHazardRules.js` rule id to its OWASP Top 10 for LLM Applications (2023 edition) category and NIST AI RMF 1.0 (January 2023) function(s) — checked by hand against the published text of both standards before shipping, not guessed and not auto-derived. A new AI-tab panel shows (1) the OWASP/NIST mapping for whatever hazards are currently triggered, and (2) the full 10-category OWASP list with covered/not-covered status for every category — e.g. LLM03 Training Data Poisoning is shown, correctly, as not covered by any rule here, since this app is a static intake form, not a training pipeline. Showing the gaps explicitly, not just the hits, is what keeps this a coverage map instead of an implied claim of completeness. Stays on the 2023 OWASP numbering for consistency with what `criteria.js`'s `GOV-5` item already cites (LLM01/LLM06), not the 2025 revision's renumbering.

**Why:** Requested as part of the same director-level pitch as #46/#47, specifically to make the existing OWASP/NIST references (currently just two inline strings — "Data Leakage Risk (OWASP LLM06)" and one checklist item's text) systematic and complete across every hazard rule. A static client-side tool cannot claim actual regulatory certification — only a coverage/mapping reference, which is exactly how this is labeled throughout the UI and in `tests/owaspNistMap.test.js` (which asserts every hazard rule has a mapping, every referenced OWASP id/NIST function is real, and the known coverage gaps are correctly reported). Latency/cost-related rules (`bounded-latency-external-llm`, `unbounded-cost-budget`, `frontier-reasoning-bounded-latency`) map to **LLM04 (Model Denial of Service)** — "attacker interacts with LLM causing exceptionally high resource consumption" is the OWASP 2023 category that actually fits latency/cost hazards; an earlier draft of this mapping incorrectly reached for LLM10 (Model Theft, which is about unauthorized IP/weights extraction, unrelated to latency/cost) and was corrected before shipping. Verified via headless browser: a clean configuration's panel correctly says there's nothing to map and lists all 10 categories with LLM03 shown uncovered; triggering the Data Leakage Risk hazard correctly surfaces LLM06 / Map, Manage; Reset correctly clears the panel back to "nothing to map."

**Trade-off:** The mapping is reviewed once, not continuously verified against either standard's text — if OWASP or NIST revises their taxonomy, this map goes stale until someone re-checks it by hand, same limitation as every other "illustrative reference, verify independently" feature in this app.

---

## 49. Problem → Solution Navigator on the portal — a table, not a 4th persona page

**Decision:** New `js/config/concernNavigatorMap.js`: an array of `{ concern, feature, url }` rows (or `{ concern, feature, status: "roadmap", note }` for the one non-shipped item), rendered as a table on `portal.html` below the 3 existing persona cards. Every row is built entirely from the existing `?sample=&framework=#tab=` deep-link mechanism (`DECISIONS.md` #35) — no new navigation plumbing, no `app.js`/state/tabs of its own, same reasoning `DECISIONS.md` #35 already used to keep the portal a single front door rather than fragmenting into more pages. 10 rows point at real, already-shipped capabilities across both apps (AI Suitability Matrix, Policy-as-Code export, Executive Health Card, AI hazard flags, Agentic Risk hazards, the OWASP/NIST coverage map, Predictive Escalation Alert, NFR Gateway Exposure, Ticket-to-Checklist Assist, sector framework presets); the 11th (Invisible Governance) is explicitly `status: "roadmap"`, rendered visibly muted with the same "not yet configured" note as `DECISIONS.md` #45, never a working-looking link. New `tests/concernNavigatorMap.test.js` asserts every non-roadmap `url` resolves to a real framework/sample/tab combination (gatekeeper-local urls checked directly against `criteria.js`'s `FRAMEWORKS`; `dor-recovery-console` urls checked against a hand-maintained mirror of that app's known tab/sample ids, since a zero-build static test suite can't import across repos) — fails loudly if content drifts from reality, same discipline as `sampleTicketText.test.js` (#41).

**Why:** Requested directly — a single reference page answering "where can I solve this problem" as a director-level product-completeness guide, distinct from the persona cards (which route by role, not by concern). Verified via headless browser: the table renders 11 rows (10 real links + 1 roadmap row); clicking the first link correctly lands on `index.html?sample=intentionally_off#tab=ai` with the AI tab active; the water sector-preset link correctly loads `Sample: Regional Water Network Upgrade`. Zero console errors.
